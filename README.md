...
- 👥 **角色库浏览**：按部门分类浏览全部角色，支持搜索、预览角色 MD 详情、导出 MD 文件、批量添加
...
*整体运行界面 — 左侧编排面板与右侧输出面板（工作流 / DAG 图 / 实时日志 / YAML）整体布局*

![自动编排视图](./images/自动编排视图.png)
*自动编排视图 — 左侧输入需求描述，右侧实时展示工作流与执行进度*

![工作流构建器](./images/工作流构建器.png)
*工作流构建器 — 从角色库中手动选择角色，拖拽构建自定义工作流*

![工作流展示效果](./images/工作流展示效果.png)
*工作流运行中 — 步骤卡片列表展示每个角色的执行状态（待执行 / 运行中 / 已完成）*

![DAG 展示效果](./images/DAG展示效果.png)
*DAG 可视化 — 运行后自动生成有向无环图，展示「开始 → 步骤1 → ... → 输出」的完整流程*

![角色库](./images/角色库.png)
*角色库浏览 — 按部门分类浏览 200+ AI 角色，支持搜索、预览详情与导出 MD 文件*
...
| GET | `/api/role/:path(*)` | 获取角色 MD 文件完整内容（预览与导出角色详情） |
...
- `renderMarkdown()` 内置简易 Markdown 渲染，支持角色详情预览（代码块、标题、列表等，无需第三方库）
